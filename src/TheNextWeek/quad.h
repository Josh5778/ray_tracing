...
class quad : public hittable {
 public:
  ...
  virtual void set_bounding_box() {
    // computer the bounding box for all four verticies.
  ...
  bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
    ...
    // not hit if the ray is parrallel to the plane.
...
inline shared_ptr<hittable_list> box(const point3& a, const point3& b,
                                     shared_ptr<material> mat) {
  // returns the 3d box containing two opposite verticies a & b

  auto sides = make_shared<hittable_list>();

  // construct two opposite verticies with minimum and maximum coordinates.
