...
class rtw_image {
 public:
  ...
  rtw_image(const char *image_filename) {
    // Loads data from the specified file. If RTW_IMAGES evironment variable is
    // defined, looks only in that directory for the image file. If the image
    // was not found, searches for the specified image file first from the
    // current directory then in the images/ subdirectory, then the _parent's_
    // images/ subdirectory, and then _that_ parent
    //, and so on for six levels up. If the image was not loaded successfully,
 ...
 private:
  ...
  void convert_to_bytes() {
    ...
    // store the resulting byte data in the 'bdata' member.
    int total_bytes = image_width * image_height * bytes_per_pixel;
    bdata = new unsigned char[total_bytes];

    // iterate through all pixel components, converting from
    // [0.0, 1.0] float values to unsigned [0, 255] byte values.
