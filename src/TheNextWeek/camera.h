...
class camera {
 public:
  ...
  double focus_dist =
      10;  // distance from camera lookfrom point to plan or perfect focus
 ...
 private:
  ...
  void initialize() {
    ...
    // calculate u,w,v unit basis vectors for the camera coordinate frame
    ...
    // viewport edges.
    vec3 viewport_u = viewport_width * u;  // vector across the horizontal edge
    vec3 viewport_v =
        viewport_height * -v;  // vector down the viewport vertical edge

    // Calculatoe the horizontal and vertical delta vectors from pixel to pixel.
  ...
  ray get_ray(int i, int j) const {
    ...
    // randomly sampled point around the pixel loction i,j.
